...
def make_map(df: pd.DataFrame,
    ...
    scatter = go.Scattergeo(
        ...
        marker_sizemin=0.2 if to_scale else 1,
    ...
    fig.update_traces(hoverlabel={'font': {'size': 18}})
...
def make_pie_charts(df: pd.DataFrame) -> Figure:
    ...
    fig = make_subplots(
        ...
        subplot_titles=['Total Area Burnt', 'Number of Incidents'])
...
app.layout = html.Div(children=[
    dbc.Tooltip('If set, the area of the circle will be '
                'equivalent to the size of the fire.',
                target='scale_switch'),
    html.Br(),
    html.H1(
    ...
    html.Div(id='selectors', children=[
        html.Div(id='sliders', children=[
            ...
            dcc.RangeSlider(
                ...
                pushable=0.1,
                step=0.1,
                marks={0: '0'} | {(i + 1): str(10 ** i)
                                  for i in range(logmax + 1)}
            )
        ], style={'padding': 20, 'flex': 1.4}),
